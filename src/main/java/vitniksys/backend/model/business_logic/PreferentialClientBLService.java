...
public class PreferentialClientBLService extends BLService {
        ...
        try {
            ...
            while (clientsIt.hasNext()) {
                ...
                while (campsIt.hasNext()) {
                    ...
                    // it is possible that the client had no participation in this camp, so no
                    // balance will be found for this camp
                    if (balance != null)
    ...
    protected void recalcBalance(Integer prefClientId, Integer campNumber) throws Exception {
        IBalanceOperator balanceOperator = BalanceOperator.getOperator();
        LeaderOperator leaderOperator = LeaderOperator.getOperator();
        ICommissionOperator commissionOperator = CommissionOperator.getOperator();

        Leader leader = leaderOperator.find(prefClientId, campNumber);

        Balance balance = new Balance(0f, 0f, 0f, 0f, 0f, 0f);
        ...
        while (devolutions.hasNext()) {
            balance.setTotalInDevolutions(
                    balance.getTotalInDevolutions() + devolutions.next().getCost());
        }
        ...
        Commission commission = commissionOperator.find(prefClientId, campNumber);
    ...
    public void registerPrefClients(List<PreferentialClient> preferentialClients) throws Exception {
        ...
        while (prefClientsIterator.hasNext()) {
            ...
            if (!PreferentialClientOperator.exist(prefClient.getId(), true))// insert only if the cp
                                                                            // not exist
            {
                ...
                try {
                    ...
                    if (leader != null) {
                        ((PreferentialClientBLServiceSubscriber) getBLServiceSubscriber())
                                .showQueriedPrefClient(leader);
                        ((PreferentialClientBLServiceSubscriber) getBLServiceSubscriber())
                                .showTotalBalance(calculateTotalBalance(id));
                    } else {
                        ((PreferentialClientBLServiceSubscriber) getBLServiceSubscriber())
                                .showNoResult("No se encuentra registrado ningún líder.");
                    }
                } catch (Exception exception) {
                    exception.printStackTrace();
                    getBLServiceSubscriber().closeProcessIsWorking(customAlert);
    ...
    public void searchSubordinatedClient(Integer id, Integer campNumber) {
        CustomAlert customAlert = getBLServiceSubscriber()
                .showProcessIsWorking("Recuperando datos del cliente preferencial " + id);
        Task<Void> task = new Task<>() {
            @Override
            protected Void call() throws Exception {
                ...
                try {
                    ...
                    if (subordinatedClient != null) {
                        ((PreferentialClientBLServiceSubscriber) getBLServiceSubscriber())
                                .showQueriedPrefClient(subordinatedClient);
                        ((PreferentialClientBLServiceSubscriber) getBLServiceSubscriber())
                                .showTotalBalance(calculateTotalBalance(id));
                    } else {
                        ((PreferentialClientBLServiceSubscriber) getBLServiceSubscriber())
    ...
    public void searchBaseClient(Integer id, Integer campNumber) {
        CustomAlert customAlert = getBLServiceSubscriber()
                .showProcessIsWorking("Recuperando datos del cliente preferencial base " + id);
        Task<Void> task = new Task<>() {
            @Override
            protected Void call() throws Exception {
                ...
                try {
                    ...
                    if (baseClient != null) {
                        ((PreferentialClientBLServiceSubscriber) getBLServiceSubscriber())
                                .showQueriedPrefClient(baseClient);
                        ((PreferentialClientBLServiceSubscriber) getBLServiceSubscriber())
                                .showTotalBalance(calculateTotalBalance(id));
                    } else {
                        ((PreferentialClientBLServiceSubscriber) getBLServiceSubscriber())
    ...
    public void searchPreferentialClients() {
        // CustomAlert customAlert = this.getServiceSubscriber().showProcessIsWorking("Espere un
        // momento mientras se realiza el proceso.");
        Task<Void> task = new Task<>() {
            @Override
            protected Void call() throws Exception {
        ...
        Task<Integer> task = new Task<>() {
            ...
            protected Integer call() throws Exception {
                ...
                try {
                    ...
                    if (order.getQuantity() > order.getReturnedQuantity()) // No client can return
                                                                           // an article if already
                                                                           // all of its units were
                                                                           // returned
                    {
                        ...
                        Commission commission = CommissionOperator.getOperator()
                                .find(prefClient.getId(), campNumber); // search for leader
                                                                       // commission. (maybe is not
                                                                       // a leader)
                        ...
                        Devolution devolution = new Devolution(devCost); // in DB, is saved the unit
                                                                         // cost.
                        devolution.setPrefClientId(order.getPrefClientId()); // Register this
                                                                             // devolution with the
                                                                             // preferential Client
                                                                             // id from the one that
                                                                             // make the order.
                        ...
                        if (commission != null) {
                            ...
                            // is why that service is in the 2nd position
                            ((CommissionBLService) getBLServiceSubscriber().getBLService(2))
                                    .updateCommission(commission, orders, repurchases); // UPDATE
                        }
                        // otherwise, register the devolution but just ignore update commission
                        // becuase, once the commission is created, it will self update.

                        getConnector().commit(); // COMMIT

                        getBLServiceSubscriber().closeProcessIsWorking(customAlert);
                        getBLServiceSubscriber().showSucces(
                                "La devolución se ha registrado exitosamente!\nCÓDIGO DE ARTÍCULO EN STOCK PARA RECOMPRA = "
                                        + devolution.getUnitCode());
                        getBLServiceSubscriber().refresh();
                    } else {
                        getBLServiceSubscriber().closeProcessIsWorking(customAlert);
        ...
        Task<Integer> task = new Task<>() {
            ...
            protected Integer call() throws Exception {
                ...
                try {
                    ...
                    recalcBalance(prefClientId, campNumber);

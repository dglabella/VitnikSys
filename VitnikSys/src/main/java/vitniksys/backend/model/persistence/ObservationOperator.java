...
public class ObservationOperator implements IObservationOperator
{
    ...
    public Observation find(Integer prefClientId, Integer campNumber) throws Exception
    {
		...
		if(resultSet.next())
		{
            ...
            leader = (Leader)LeaderOperator.getOperator().findShort(prefClientId);
            if(leader == null)
                baseClient = (BaseClient)BaseClientOperator.getOperator().findShort(prefClientId);

            camp = CampaignOperator.getOperator().find(campNumber);
			...
			//Associations
            ret.setPrefClient(leader != null ? leader : baseClient);
			ret.setCamp(camp);
		}

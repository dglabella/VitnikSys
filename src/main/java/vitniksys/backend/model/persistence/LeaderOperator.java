...
public class LeaderOperator extends BaseClientOperator
{
    ...
    public Integer insert(PreferentialClient cp) throws Exception
    {
        ...
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) "+
        "ON DUPLICATE KEY UPDATE `dni` = ?, `nombre` = ?, `apellido` = ?, `lugar` = ?, `fecha_nac` = ?, `email` = ?, `tel` = ?, `es_lider` = ?;";
        ...
        if(cp.getBirthDate() != null)
        {
            ...
            statement.setDate(14, Date.valueOf(cp.getBirthDate()));
        }
        else
        ...
        if(cp.getEmail() != null && !cp.getEmail().isBlank())
        {
            ...
            statement.setString(15, cp.getEmail());
        }
        else
        ...
        if(cp.getPhoneNumber() != null)
        {
            ...
            statement.setLong(16, cp.getPhoneNumber());
        }
        else

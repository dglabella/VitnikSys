...
public class MainMenuViewCntlr extends TableViewCntlr
        implements PreferentialClientBLServiceSubscriber {
    private int PREF_CLIENTS_TABLE_NUMBER;
    private String COMMAND;

    // ================================= FXML variables =================================
    @FXML
    private TableView<PreferentialClient> prefClients;

    @FXML
    private TableColumn<PreferentialClient, Integer> id;
    @FXML
    private TableColumn<PreferentialClient, Long> dni;
    @FXML
    private TableColumn<PreferentialClient, String> name;
    @FXML
    private TableColumn<PreferentialClient, String> lastName;
    @FXML
    private TableColumn<PreferentialClient, String> email;
    @FXML
    private TableColumn<PreferentialClient, Long> phoneNumber;
    ...
    private void summaryButtonPressed() {
        ViewCntlr viewCntlr = this.createStage("Resumen de clientes", "summary",
                new PreferentialClientBLService(), new CampaignBLService());
    ...
    private void prefClientsAutoLoadButtonPressed() {
        ...
        if (cpsFile != null) {
            if (FilenameUtils.getExtension(cpsFile.getName())
                    .equalsIgnoreCase(App.ConstraitConstants.DETAIL_FILE_EXTENSION)) {
                new CustomAlert(AlertType.CONFIRMATION, "CONFIRMAR", "El archivo seleccionado es: "
                        + cpsFile.getAbsolutePath()
                        + "\nEsta seguro que desea cargar este archivo de clientes preferenciales?")
                                .customShow().ifPresent(response -> {
                                    if (response == ButtonType.OK) {
                                        try {
                                            ((PreferentialClientBLService) this.getBLService(0))
                                                    .registerPrefClientsAuto(cpsFile);
    ...
    private void catButtonOnMousePressed() throws Exception {
        ViewCntlr viewCntlr =
                this.createStage("Consultar Cátalogo", "catalogueQuery", new CatalogueBLService());
        viewCntlr.getStage().show();
        viewCntlr.manualInitialize();
    }
    ...
    private void searchCampButtonPressed() {
        ViewCntlr viewCntlr =
                this.createStage("Consultar campaña", "searchCamps", new CampaignBLService());
        viewCntlr.getStage().show();
        viewCntlr.manualInitialize();
    }
    ...
    private void newCpButtonPressed() {
        this.createStage("Formulario de registro de Cliente preferencial", "clientRegister",
                new PreferentialClientBLService()).getStage().show();
    }

    @FXML
    private void newCampButtonPressed() {
        ViewCntlr viewCntlr = this.createStage("Crear campaña", "campRegister",
                new CampaignBLService(), new CatalogueBLService());
        viewCntlr.getStage().show();
        viewCntlr.manualInitialize();
    }
    ...
    private void addDetailFileButtonPressed() {
        ...
        if (detail != null) {
            if (FilenameUtils.getExtension(detail.getName())
                    .equalsIgnoreCase(App.ConstraitConstants.DETAIL_FILE_EXTENSION)) {
                new CustomAlert(AlertType.CONFIRMATION, "CONFIRMAR",
                        "El archivo seleccionado es: " + detail.getAbsolutePath()
                                + "\nEsta seguro que desea cargar este detalle?").customShow()
                                        .ifPresent(response -> {
                                            if (response == ButtonType.OK) {
                                                if (this.generateDataBaseBackUp(COMMAND)) {
                                                    ((CampaignBLService) this.getBLService(1))
                                                            .registerOrders(detail);
                                                } else {
                                                    new CustomAlert(AlertType.ERROR, "ERROR",
                                                            "No se pudo realizar la copia de seguridad de la base de datos.")
                                                                    .customShow();
                                                    new CustomAlert(AlertType.CONFIRMATION,
                                                            "CONFIRMAR",
                                                            "Desea cargar de todos modos el detalle?\n"
                                                                    + "IMPORTANTE: Debe estar seguro que el archivo que esta cargando es correcto.\n(archivo detalle nunca antes cargado o "
                                                                    + "archivo detalle sin falta de información)")
                                                                            .customShow().ifPresent(
                                                                                    otherResponse -> {
                                                                                        if (otherResponse == ButtonType.OK) {
                                                                                            ((CampaignBLService) this
                                                                                                    .getBLService(
                                                                                                            1)).registerOrders(
                                                                                                                    detail);
                                                                                        }
    ...
    private void openManagementButtonPressed() {
        if (this.prefClients.getSelectionModel().getSelectedIndex() > -1) {
            PreferentialClient selectedPrefClient = this.prefClients.getItems()
                    .get(this.prefClients.getSelectionModel().getSelectedIndex());

            ViewCntlr viewCntlr = this.createStage("Gestión de cliente preferencial",
                    "clientManagement", new PreferentialClientBLService(), new CampaignBLService(),
                    new CommissionBLService());
            viewCntlr.getStage().show();
            ...
            this.filterTable(this.PREF_CLIENTS_TABLE_NUMBER, new Predicate<PreferentialClient>() {
                ...
                public boolean test(PreferentialClient prefClient) {
                    ...
                    if (newValue.isBlank() || String.valueOf(prefClient.getId()).contains(newValue)
                            || (prefClient.getDni() != null
                                    && String.valueOf(prefClient.getDni()).contains(newValue))
                            || prefClient.getName().contains(newValue.toUpperCase())
                            || String.valueOf(prefClient.getLastName())
                                    .contains(newValue.toUpperCase())
                            || (prefClient.getEmail() != null
                                    && prefClient.getEmail().contains(newValue))
                            || (prefClient.getPhoneNumber() != null && String
                                    .valueOf(prefClient.getPhoneNumber()).contains(newValue))) {
                        ret = true;
                    } else {
                        ret = false;
                    }
